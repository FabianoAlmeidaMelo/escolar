...
CREATE OR REPLACE VIEW view_inadimplentes AS 
 SELECT
  ...
  WHERE pagamento.efet = false 
  AND pagamento.data < now()::date
  AND contrato.id is NOT NULL
  GROUP BY pagamento.escola_id, pagamento.contrato_id, pessoa.cpf, pessoa.nome, pessoa.celular, pessoa.email, aluno.nome, contratoaluno.serie_id;
